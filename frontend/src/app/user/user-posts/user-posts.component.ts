...
export class UserPostsComponent implements OnInit {
  ...
  removePost(post: Post, index: number): void {
    console.log(post);
    const result = confirm('Are you sure you want to delete this publication?');
    if (result) {
  ...
  toggleComments(postIndex: number): void {
    const post = this.posts[postIndex];
    post.showComments = !post.showComments; // Toggle the comments visibility
}
